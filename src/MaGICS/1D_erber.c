/*Photons emmited in bins, not considering influence of
hard photon emmision on soft photon emmision*/
void propagate_1D_erberaprox()
{
  struct particle *aux,*next;
  double dt,deltat=0,chi_now;/*dt: time step; Dt: aux's time flight*/
  double proba;
  int i;
  ...
  for (aux=first_particle;aux!=NULL;aux=next)
	...
	for(timer=0;timer<time_divisor;timer++)
	{
	  ...
	  for (eta=1;eta>1E-10;eta*=energy_slice)
	    {
	      photon_number=photon_number_erber(aux->energy,eta*sqrt(energy_slice),
